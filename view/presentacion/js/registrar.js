...
/**
 * MÉTODO PARA REGISTRAR LA MATERIA PRIMA
 */
 $(document).ready(function () {
    $("#FormRegistrarMateriaPrima").on('submit', function (e) {
        e.preventDefault();
        var datos = $(this).serializeArray();
        $.ajax({
            url: 'model/registrarMateriaPrima.php',
            data: datos,
            type: $(this).attr("method"),
            dataType: "json",
            beforeSend: function () {
            ...
            success: function (data) {
                if (data.respuesta == 'exito') {
                    ingresoExitoso("Exito!", "Se registro correctamente la materia prima.");
                    setTimeout(function () {
                        location.reload();
                    }, 1000);
                } else if (data.respuesta == 'error') {
 ...
 $(document).ready(function () {
    $("#FormRegistrarProducto").on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.respuesta == 'exito') {
                    ingresoExitoso("Exito!", "Se registro correctamente el producto.");
                    setTimeout(function () {
                        location.reload();
                    }, 1000);
                } else if (data.respuesta == 'error') {
